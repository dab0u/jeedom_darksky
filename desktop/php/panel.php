...
<script src="plugins/darksky/desktop/js/skycons.js"></script>

<link rel="stylesheet" type="text/css" href="plugins/darksky/desktop/weather-icons/css/weather-icons.min.css" />

<div class="container-fluid darksky-panel">
	<div class="row">
		<div class="col-md-12">
			<div class="btn-group">
				...
				<?php
				$first = 0;
				$eqLogics = darksky::byType('darksky', true);
				foreach ($eqLogics as $darksky) {
					if ($first == 0 ) {
						$selected = $darksky->getId();
						$first = 1;
					}
					echo '<button class="btn btn-default darkskyEqlogic" id="' . $darksky->getId() . '" type="button" onClick="loadingData(' . $darksky->getId() . ')">' . $darksky->getName() . '</button>';
				}
				?>
			</div>
		</div>
...
$(function () {
	...
	function loadingData(eqLogic){
		...
		url: "plugins/darksky/core/ajax/darksky.ajax.php", // url du fichier php
		data: {
			action: "loadingData",
			value: eqLogic,
		},
		...
		$('.darkskyEqlogic').removeClass('btn-success');
		$('.darkskyEqlogic').addClass('btn-default');
